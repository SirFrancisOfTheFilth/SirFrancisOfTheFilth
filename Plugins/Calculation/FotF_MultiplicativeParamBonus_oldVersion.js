...
FotF_ParamFactorSettings = {
    ...
    skillInfoBonusWidth: 166, //Bonus width for skill info window (can also be negative)
    itemInfoBonusWidth: 170, //Bonus width for item info window
    terrainWindowBonusWidth: 194, //Bonus width for terrain window
    lineSpacing: 20, //Spacing before new line begins (in pixels)
    itemWindowHeightFactor: 0.5 //Lower this to shorten the item window, increase to expand it when paramFactor info is drawn. I'm too lazy to figure this shit out, sue me.
};
...
(function () {
    ...
    SkillInfoWindow.drawWindowContent = function (x, y) {
        ...
        y += FotF_ParamFactorSettings.lineSpacing;
        ...
        if (this._aggregationViewer !== null) {
            ...
            if (count !== 0) {
                ...
                y += FotF_ParamFactorSettings.lineSpacing * this._aggregationViewer.getAggregationViewerCount();
            }
        }
        ...
        if (typeof this._skill.custom.paramFactorRegen === 'number' && this._skill.custom.paramFactorRegen !== 0) {
            ...
            y += FotF_ParamFactorRenderer.getRecoverySpacing(this._skill, 0, FotF_ParamFactorSettings.lineSpacing);
        }
    ...
    SkillInfoWindow.getWindowHeight = function () {
        ...
        count += FotF_ParamFactorRenderer.getRecoverySpacing(this._skill, 0, FotF_ParamFactorSettings.lineSpacing) + FotF_ParamFactorRenderer.getParamSpacing(this._skill, 0, FotF_ParamFactorSettings.lineSpacing);

        return count;
    };

    //Increase skill info window width if mult bonus/regen is displayed
    var FotF_ExtendSkillInfoWidthtMult = SkillInfoWindow.getWindowWidth;
    SkillInfoWindow.getWindowWidth = function () {
        var width = FotF_ExtendSkillInfoWidthtMult.call(this);
    ...
    MapParts.Terrain._drawContent = function (x, y, terrain) {
        ...
        y += (this._getPartsCount(terrain) - this._getPartsCountFactor(terrain) - 1) * this.getIntervalY();
        ...
        y += FotF_ParamFactorRenderer.getRecoverySpacing(terrain, 0, FotF_ParamFactorSettings.lineSpacing);
...
MapParts.Terrain._getPartsCountFactor = function (terrain) {
    ...
    //Interval of 1 to get number of lines to space and subtract 1 for whatever fucking reason, it works now
...
ItemSentence.ParamFactor = defineObject(BaseItemSentence, {
    ...
    getItemSentenceCount: function (item) {
        var count = FotF_ParamFactorRenderer.getParamSpacing(item, 0, 1) + FotF_ParamFactorRenderer.getRecoverySpacing(item, 0, 1);
        count = Math.ceil(count * FotF_ParamFactorSettings.itemWindowHeightFactor);
        root.log(count);
        return count;
    }
});
...
FotF_ParamFactorRenderer = {
    drawParamBonus: function (x, y, obj) {
        ...
        ItemInfoRenderer.drawKeyword(x, y, FotF_ParamFactorSettings.bonusKeyword);
        bonusX = x + ItemInfoRenderer.getSpaceX();
        y -= 3;
        ...
        for (i = 0; i < count; i++) {
            ...
            if (!FotF_ParamFactorControl.isScalingFactor(obj)) {
                ...
                if (typeof obj.custom === 'object' && typeof obj.custom.surroundedBonus === 'number') {
                    ...
                    x += 17;
                    TextRenderer.drawKeywordText(x, y, FotF_ParamFactorSettings.adjacentString, -1, color, font);
                    x += TextRenderer.getTextWidth(FotF_ParamFactorSettings.adjacentString, font) + 4;
                    NumberRenderer.drawRightNumber(x, y, obj.custom.surroundedBonus);
                    x += obj.custom.surroundedBonus.toString().length * 11;
                    TextRenderer.drawKeywordText(x, y, FotF_ParamFactorSettings.adjacentString2, -1, color, font);
                    y += FotF_ParamFactorSettings.lineSpacing;
                }
                ...
                if (typeof obj.custom === 'object' && typeof obj.custom.healthBonus === 'number') {
                    ...
                    if (divider < 0) {
                        divider *= -1;
                        x += 17;
                        TextRenderer.drawKeywordText(x, y, FotF_ParamFactorSettings.mHealthStrings[0], -1, color, font);
                        x += 10;
                        NumberRenderer.drawRightNumber(x, y, divider);
                        x += divider.toString().length * 11;
                        TextRenderer.drawSignText(x, y, '%');
                        x += 10;
                        TextRenderer.drawKeywordText(x, y, FotF_ParamFactorSettings.mHealthStrings[1], -1, color, font);
                    } else if (divider > 0) {
                        x += 17;
                        TextRenderer.drawKeywordText(x, y, FotF_ParamFactorSettings.healthStrings[0], -1, color, font);
                        x += 10;
                        NumberRenderer.drawRightNumber(x, y, divider);
                        x += divider.toString().length * 11;
                        TextRenderer.drawSignText(x, y, '%');
                        x += 10;
                        TextRenderer.drawKeywordText(x, y, FotF_ParamFactorSettings.healthStrings[1], -1, color, font);
                    }
                ...
                if (typeof obj.custom === 'object' && typeof obj.custom.customBonus === 'function') {
                    ...
                    if (typeof string1 === 'string') {
                        TextRenderer.drawKeywordText(x, y, string1, -1, color, font);
                        x += TextRenderer.getTextWidth(string1, font) + 4;
                    }
                    if (typeof value === 'number') {
                    ...
                    if (typeof string2 === 'string') {
                        TextRenderer.drawKeywordText(x, y, string2, -1, color, font);
                    }
    ...
    drawAbilityBonus: function (x, y, obj) {
        ...
        //y += FotF_ParamFactorSettings.lineSpacing;
        ItemInfoRenderer.drawKeyword(x, y, FotF_ParamFactorSettings.abilityBonusKeyword);
        bonusX = x + ItemInfoRenderer.getSpaceX();
    ...
    //obj is terrain
    drawTerrainBonus: function (x, y, obj, interval) {
        var i;
        var count = 3;
        var textui = ItemInfoRenderer.getTextUI();
        var color = textui.getColor();
        ...
        for (i = 0; i < count; i++) {
            ...
            if (!FotF_ParamFactorControl.isScalingFactor(obj)) {
                x = bonusX;
                text = this.getTerrainFactorName(i);
                TextRenderer.drawKeywordText(x, y, text, -1, color, font);
    ...
    drawRecovery: function (x, y, obj) {
        ...
        ItemInfoRenderer.drawKeyword(x, y, FotF_ParamFactorSettings.recoveryString);
        bonusX = x + ItemInfoRenderer.getSpaceX() - 5;
