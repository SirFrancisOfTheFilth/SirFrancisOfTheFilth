...
2025/11/18
Fixed no return value on item info line count function

2026/01/03
Fixed subsequent stat cap items not working if one was already used on that
unit

Fixed stat caps adding the default cap as well

Fixed calculated caps being able to assume negative values
...
var FotF_StatCapControl = {
    ...
    pushEntry: function (unit, arr) {
        ...
        if (entryIndex < 0) {
            ...
            for (i = 0; i < oldEntry.stats.length; i++) {
                var mod = arr[i];
    ...
    getValidBonusArray: function (unit, item) {
        ...
        for (i = 0; i < arr.length; i++) {
            ...
            if (stat > 0 && combined > cap) {
                stat = combined - cap;
            }

            if (stat < 0 && combined < 0) {
                stat = max * -1;
            }
