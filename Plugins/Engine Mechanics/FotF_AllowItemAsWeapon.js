...
2025/02/03
Fixed crash in conjunction with Enemy Canto plugin when checking
if enemy can move again.

2026/01/25
Added null check to AttackChecker._isWeaponEnabled alias for compatibility
...
UnitCommand.Attack.isRepeatMoveAllowed = function () {
    var unit = null;
    //Crash prevention
    if (typeof this._listCommandManager === 'undefined' || this._listCommandManager === null) {
        unit = root.getCurrentSession().getActiveEventUnit(); //This is so stupid but it works
    } else {
    ...
    //So if AI does it with enemy canto plugin, they can actually canto
    if (unit.getUnitType() !== UnitType.PLAYER || !StateControl.isTargetControllable(unit)) {
        return true;
    }
