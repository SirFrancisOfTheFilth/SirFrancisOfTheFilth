...
2020-12-11: Modified by Francis of the Filth
- added blue targeting lines in opposite direction to visualize possible player attacks
...
var MOVE_ALLOW_SETTING = {
	...
	EndPoint: {
		//画像ファイル内における終端部分の矢印のオフセット[x座標, y座標]
	...
	Line: {
		//画像ファイル内における中間部分の矢印のオフセット[x座標, y座標]
	...
	StartPoint: {
		//画像ファイル内における始点部分の矢印のオフセット[x座標, y座標]
...
(function () {
	...
	MapSequenceArea._drawMoveUnit = function (direction) {
		...
		if (this._movementLocus.movePoint != null && this._isTargetMovable() && !ConfigItem.MovePointVisible.isDisabled()) {
			...
			if (remainingMove > 0 && remainingMove < this._unitMovParameter) {
				NumberRenderer.drawNumber(x2 + 10 - root.getCurrentSession().getScrollPixelX(), y2 + 3 - root.getCurrentSession().getScrollPixelY(), remainingMove);
			}
		}
	...
	MapSequenceArea._moveArea = function () {
		...
		if ((this._prevMapCursorMapX !== currentMapCursorX || this._prevMapCursorMapY !== currentMapCursorY) && this._isTargetMovable() === true) {
	...
	MapSequenceArea._setupTargetingLines = function (goalIndex) {
		...
		var enemyUnit;
		var playerUnit = this._targetUnit;
		if (this._simulator2 == null) this._simulator2 = root.getCurrentSession().createMapSimulator();

		var attackRangePlayer = UnitRangePanel.getUnitAttackRange(playerUnit);
		var startRangePlayer = attackRangePlayer.startRange;
		var endRangePlayer = attackRangePlayer.endRange;
		var playerMov = attackRangePlayer.mov; // ParamBonus.getMov(enemyUnit)
		var playerHasWeapon = attackRangePlayer.endRange !== 0;
		var enemyArray = [];

		//FotF edit to include lines from player to enemy
		if (playerHasWeapon) {
			this._simulator2.startSimulationWeapon(playerUnit, playerMov, startRangePlayer, endRangePlayer);
			var arr = IndexArray.getBestIndexArray(currentMapCursorX, currentMapCursorY, startRangePlayer, endRangePlayer);
			for (var i = 0; i < arr.length; i++) {
				var targetEnemy = PosChecker.getUnitFromPos(CurrentMap.getX(arr[i]), CurrentMap.getY(arr[i]));
				if (targetEnemy !== null && targetEnemy.getUnitType() === UnitType.ENEMY) {
					enemyArray.push(targetEnemy);
				}
			}
		...
		for (var i = 0; i < enemyArray.length; i++) {
			var targetEnemy = enemyArray[i];

			var arrow = defineObject(BaseShape, {
				_figure: null,
				_color: 0x0000ff, // colour of arrow
				_alpha: 128, // opacity of arrow
				_strokeColor: 0x0000ff, // colour of arrow outline
			...
			// e for enemy, p for cursor
			var ex = LayoutControl.getPixelX(targetEnemy.getMapX());
			var ey = LayoutControl.getPixelY(targetEnemy.getMapY());
			...
			//END EDIT
			arrow.setupShape(cx + 2, cy + 2, ex + 2, ey + 2);
		...
		for (var e = 0; e < ListE.getCount(); e++) {
			...
			var pattern = enemyUnit.getAIPattern();
			...
			if (enemyHasWeapon) {
				...
				if (inEnemyRange) {
					...
					//END EDIT
					arrow.setupShape(ex - 2, ey - 2, cx - 2, cy - 2);
					this._targetLineArray.appendObject(arrow);
				}
			}
		}
	...
	MapSequenceArea._drawLineMovementLocus = function (x, y, next_id, direction_id) {
		if (!ConfigItem.MovementArrow.isDisabled()) {
			if ((direction_id === DirectionType.LEFT && next_id === DirectionType.BOTTOM) || (direction_id === DirectionType.TOP && next_id === DirectionType.RIGHT)) {
				// 下方向-右方向曲線を描画
			...
			} else if ((direction_id === DirectionType.LEFT && next_id === DirectionType.TOP) || (direction_id === DirectionType.BOTTOM && next_id === DirectionType.RIGHT)) {
				// 上方向-右方向曲線を描画
			...
			} else if ((direction_id === DirectionType.RIGHT && next_id === DirectionType.TOP) || (direction_id === DirectionType.BOTTOM && next_id === DirectionType.LEFT)) {
				// 上方向-左方向曲線を描画
			...
			} else if ((direction_id === DirectionType.TOP && next_id === DirectionType.LEFT) || (direction_id === DirectionType.RIGHT && next_id === DirectionType.BOTTOM)) {
				// 下方向-左方向曲線を描画
			...
			} else if ((direction_id === DirectionType.LEFT && next_id === DirectionType.LEFT) || (direction_id === DirectionType.RIGHT && next_id === DirectionType.RIGHT)) {
				// 横直線を描画
			...
			} else if ((direction_id === DirectionType.TOP && next_id === DirectionType.TOP) || (direction_id === DirectionType.BOTTOM && next_id === DirectionType.BOTTOM)) {
				// 縦直線を描画
