	...
	for (t = 0; t < finalList.getCount(); t++) {
		...
		for (i = 0; i < list.getCount(); i++) {
			...
			if (unit.getUnitType() !== UnitType.PLAYER && !unit.custom.aboveThird) {
				...
				if ((terrain.custom.StealthCL || terrainEx.custom.StealthCL) && !terrain.custom.noHide && !terrainEx.custom.noHide && cutArr.indexOf(index) < 0 && unit.isInvisible() === false && FotF_LayerRendererConstants.HideUnitsOutOfRange === true && invisIndex < 0) {
					hiddenUnitArray.push(unit);
					unit.setInvisible(true);
					root.log(unit.getName() + ' set invisible')
				}
				if ((cutArr.indexOf(index) > - 1 || FotF_LayerRendererConstants.HideUnitsOutOfRange === false || (!terrain.custom.StealthCL && !terrainEx.custom.StealthCL)) && unit.isInvisible() === true && blockAfterImage2 !== unit.getId() && invisIndex < 0) {
