...
I added a new function! (The old ones still work, don't worry)
You can specify coordinates and optinally tile information to remove all tiles
(or just the specified kind) from the position:

FotF_DeleteAdditionalLayerTileByPos = function(isRuntime, id, xSrc, ySrc, xDest, yDest);

and

FotF_DeleteAdditionalThirdLayerTileByPos = function(isRuntime, id, xSrc, ySrc, xDest, yDest);

If you set the first 4 parameters to null, all tiles on these coordinates will be
removed. Tiles removed with this function are deleted from their arrays, so you'll
have to add them in again if you want them back.
...
However, this breaks the see-through function of these tiles.
...
2024/07/15
Fixed imported layers carrying over to the next map

2024/10/20
Added functions to remove specific tiles or all tiles from specific coordinates,
see "That's cool, but what if I want to erase the additional mapchips?"
...
(function () {
	...
	/*MapLayer.prepareMapLayer*/CurrentMap.prepareMap = function () {
		...
		*/
		...
		FotF_ImportArray = null;
		FotF_ImportArrayThird = null;
...
//Deletes a specified additional Layer tile (or all) from position xDest/yDest
var FotF_DeleteAdditionalLayerTileByPos = function(isRuntime, id, xSrc, ySrc, xDest, yDest) {
	var i;
	var arr = FotF_AdditionalLayerArray;
	
	for (i = 0; i < arr.length; i++) {
		if (xDest === arr[i][4] && yDest === arr[i][5]) {
			if ((isRuntime === arr[i][0] || isRuntime === null) && (id === arr[i][1] || id === null) && (xSrc === arr[i][2] || xSrc === null) && (ySrc === arr[i][3] || ySrc === null)) {
				FotF_AdditionalLayerArray.splice(i, 1);
			}
		}
	}
};

//Deletes a specified additional third layer tile (or all) from position xDest/yDest
var FotF_DeleteAdditionalLayerTileByPos = function(isRuntime, id, xSrc, ySrc, xDest, yDest) {
	var i;
	var arr = FotF_AdditionalThirdLayerArray;
	
	for (i = 0; i < arr.length; i++) {
		if (xDest === arr[i][4] && yDest === arr[i][5]) {
			if ((isRuntime === arr[i][0] || isRuntime === null) && (id === arr[i][1] || id === null) && (xSrc === arr[i][2] || xSrc === null) && (ySrc === arr[i][3] || ySrc === null)) {
				FotF_AdditionalThirdLayerArray.splice(i, 1);
			}
		}
	}
};
