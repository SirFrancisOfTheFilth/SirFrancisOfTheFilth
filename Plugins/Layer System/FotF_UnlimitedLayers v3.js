...
1. 	By far the easiest way is to use the export/import feature. This allows you
	export the information inside a "dummy map" to a file and re-import the
...
.txt extension of course) and put it into the FotF_LayerSystem folder inside
...
manual addition and finally those added in-game through the functions.
...
(function () {
	...
	var FotF_PrepareMapLayerUnlimited = MapLayer.prepareMapLayer;
	...
	MapLayer.drawMapLayer = function () {
		...
		if ((FotF_ImportArray === null || FotF_ImportArrayThird === null) && session !== null) {
	...
	LoadSaveScreen._getCustomObject = function() {
		...
		if (FotF_AdditionalLayerArray.length !== 0) {
			obj.addArr = FotF_AdditionalLayerArray;
			//root.log('added array to object');
		}
		
		if (FotF_AdditionalThirdLayerArray.length !== 0) {
			obj.addArr2 = FotF_AdditionalThirdLayerArray;
		}
		
		if (FotF_DisableAdditionalLayerArray.length !== 0) {
			obj.disArr = FotF_DisableAdditionalLayerArray;
		}
		
		if (FotF_DisableAdditionalThirdLayerArray.length !== 0) {
			obj.disArr2 = FotF_DisableAdditionalThirdLayerArray;
		}
		
		if (FotF_AdditionalAnimeArray1.length !== 0) {
			obj.animArr1 = FotF_AdditionalAnimeArray1;
		}
		
		if (FotF_AdditionalAnimeArray2.length !== 0) {
			obj.animArr2 = FotF_AdditionalAnimeArray2;
		}
		
		if (FotF_AsyncAnimeArray1.length !== 0) {
			obj.animArr3 = FotF_AsyncAnimeArray1;
		}
		
		if (FotF_AsyncAnimeArray2.length !== 0) {
	...
	for (x = 0; x <= width; x++) {
		for (y = 0; y <= height; y++) {
			...
			if (handle2 !== null) {
				...
				if (isRuntime === ignoreIsRuntime && ID === ignoreID && xSrc === ignoreXSrc && ySrc === ignoreYSrc) {
					var string2 = null;
				} else {
