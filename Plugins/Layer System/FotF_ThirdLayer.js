...
____________________________________________________________________________ 
  ...
  Firstly, this is intended to be used in conjunction with the Rest of the Layer System
  (FotF_LayerRenderer.js and FotF_UnlimitedLayers.js). Please also read the instructions
  of these files.
  ...
  - It's possible to hide enemies and allies out of unit range and behind third layer tiles.
	They become visible again once they're in range of a player unit.
  
  
  To combat units instantly becoming visible again if turned invisible through event commands,
  you have to use a special function through an "Execute Script" event command instead. Make
  sure to check "Execute Code" and write the following into the text prompt:
  
  FotF_ToggleUnitInvisibility(unitID, unitType, toggle);												//unitID is the ID of the unit as seen in the database (even for enemies etc.)
																										//unitType is the type of the unit. This can be one of the following:
																										
																													UnitGroup.PLAYER					//Players tab of the unit screen
																													UnitGroup.ENEMY						//Enemies tab of the unit screen
																													UnitGroup.ENEMYEVENT				//Event Enemies tab of the unit screen
																													UnitGroup.ALLY						//Allies tab of the unit screen
																													UnitGroup.ALLYEVENT					//Even Allies tab of the unit screen
																													UnitGroup.REINFORCE					//Reinforcements tab of the unit screen
																													UnitGroup.GUEST						//Guests tab of the unit screen
																													UnitGroup.GUESTEVENT				//Event Guests tab of the unit screen
																													
																										//toggle is either true for setting the unit invisible or false for making it visible again
																										
																										
  Example: If you want the enemy (not event enemy) with ID 3 to be invisible,
  you'd use the function like this:
  
	FotF_ToggleUnitInvisibility(3, UnitGroup.ENEMY, true);
	
  Now if you want them to become visible again, simply write:
  
	FotF_ToggleUnitInvisibility(3, UnitGroup.ENEMY, false);
  ...
  2020/10/05 Released
	...
	2024/04/02
		Fixed unit heads being cut off if using "Unit Appearance" event command.
		
	2024/04/10
		Fixed some weirdness with enemy units hidden by the third layer not reappearing if they move
		to non-third layer tiles.
		
	2024/05/05
		Fixed afterimages of enemies and allies during movement
		
		Fixed (somewhat) units reverting from the invisible state if they're made invisible through
		event commands. Instead of using the "Change Unit State" event command, use an "Execute Script"
		event command with the function I provided you with. This is the only way to properly do this,
		sorry.
		
	2024/05/16
		Added custom parameter noHide which will disable the unit hiding feature for certain tiles only,
		useful for e.g. windows, cell bars or other see-through tiles.
		...
		for (t = 0; t < finalList.getCount(); t++) {
			...
			for (i = 0; i < list.getCount(); i++) {
				...
				if (unit.getUnitType() !== UnitType.PLAYER && !unit.custom.aboveThird) {
					...
					if ((terrain.custom.StealthCL || terrainEx.custom.StealthCL) && cutArr.indexOf(index) < 0 && unit.isInvisible() === false && FotF_LayerRendererConstants.HideUnitsOutOfRange === true) {
						hiddenUnitArray.push(unit);
						unit.setInvisible(true);
						root.log(unit.getName() + ' set invisible')
					}
					if ((cutArr.indexOf(index) > - 1 || FotF_LayerRendererConstants.HideUnitsOutOfRange === false || (!terrain.custom.StealthCL && !terrainEx.custom.StealthCL)) && unit.isInvisible() === true && blockAfterImage2 !== unit.getId()) {
						unit.setInvisible(false);
						root.log(unit.getName() + ' set visible')
						hiddenUnitArray.splice(t, 1);
					}
					
				} else if (terrain2 !== null && terrain2.custom.StealthCL && !terrain.custom.StealthCL && blockAfterImage === false && !unit.custom.aboveThird) {
				...
				} else if (unit.custom.aboveThird && blockAfterImage2 !== unit.getId()) {
