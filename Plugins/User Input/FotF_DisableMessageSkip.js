...
Do not use non-self-advancing messages (ones without \at) while the switch is
activated! This will softlock the player and they will have to either close
the game and lose their progress or skip the event!

Also you should never forget to deactivate the switch as it persists even
...
(function () {
	...
	InputControl.isSystemState = function() {
		var switchTable = root.getMetaSession().getGlobalSwitchTable();
		var switchIndex = switchTable.getSwitchIndexFromId(FotF_DisableSkipSwitch);
		if (MessageAnalyzer._waitChain !== null && switchTable !== null) {
			if (switchTable.isSwitchOn(switchIndex) === true && MessageAnalyzer._waitChain.isAutoMode() && !MessageAnalyzer._waitChain.isPageAutoChange()) {
				return false;
			}
		}
