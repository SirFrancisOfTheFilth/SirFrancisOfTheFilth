	...
	if (root.getCurrentSession() !== null) {
		...
		for (t = 0; t < arr.length; ++t) {
			if (PosChecker.getUnitFromPos(arr[t][1], arr[t][2]) !== null && PosChecker.getUnitFromPos(arr[t][1], arr[t][2]).custom.aboveThird) {
				...
				root.drawCharChipSymbol(arr[t][1] * GraphicsFormat.MAPCHIP_WIDTH - scrollPixelX, (arr[t][2]) * GraphicsFormat.MAPCHIP_HEIGHT - scrollPixelY, unit);
				
				if (hpType === 0 || hpType === 1) {
					root.drawCharChipHPGauge(arr[t][1] * GraphicsFormat.MAPCHIP_WIDTH - scrollPixelX, (arr[t][2]) * GraphicsFormat.MAPCHIP_HEIGHT - scrollPixelY, unit);
				}
				
				root.drawCharChipStateIcon(arr[t][1] * GraphicsFormat.MAPCHIP_WIDTH - scrollPixelX, (arr[t][2]) * GraphicsFormat.MAPCHIP_HEIGHT - scrollPixelY, unit);
...
var FotF_RenderThirdLayerScrollUnit = function (unit, x, y, unitRenderParam) {
	...
	for (t = 0; t < arr.length; ++t) {
		...
		if (PosChecker.getUnitFromPos(arr[t][1], arr[t][2]) !== null && PosChecker.getUnitFromPos(arr[t][1], arr[t][2]).custom.aboveThird) {
			...
			var unit2 = PosChecker.getUnitFromPos(arr[t][1], arr[t][2])
			var scrollPixelX = root.getCurrentSession().getScrollPixelX();
			var scrollPixelY = root.getCurrentSession().getScrollPixelY();
			var directionArray = [4, 1, 2, 3, 0];
			...
			unitRenderParam.animationIndex = 0
			if (unitRenderParam.colorIndex === -1) {
				unitRenderParam.colorIndex = unit2.getUnitType();
			}
			if (unit2.isWait()) {
				unitRenderParam.colorIndex = 3;
			}
			if (unitRenderParam.handle === null) {
				unitRenderParam.handle = unit2.getCharChipResourceHandle();
			}
			unitRenderParam.direction = unit2.getDirection();
			unitRenderParam.animationIndex = MapLayer.getAnimationIndexFromUnit(unit2);
			...
			UnitRenderer._drawCustomCharChip(unit2, arr[t][1] * GraphicsFormat.MAPCHIP_WIDTH - scrollPixelX, (arr[t][2]) * GraphicsFormat.MAPCHIP_HEIGHT - scrollPixelY, unitRenderParam);
			
			root.drawCharChipSymbol(arr[t][1] * GraphicsFormat.MAPCHIP_WIDTH - scrollPixelX, (arr[t][2]) * GraphicsFormat.MAPCHIP_HEIGHT - scrollPixelY, unit2);
				
			if (hpType === 0 || hpType === 1) {
				root.drawCharChipHPGauge(arr[t][1] * GraphicsFormat.MAPCHIP_WIDTH - scrollPixelX, (arr[t][2]) * GraphicsFormat.MAPCHIP_HEIGHT - scrollPixelY, unit2);
			}
				
			root.drawCharChipStateIcon(arr[t][1] * GraphicsFormat.MAPCHIP_WIDTH - scrollPixelX, (arr[t][2]) * GraphicsFormat.MAPCHIP_HEIGHT - scrollPixelY, unit2);
		}
